...
<table class="table table-striped table-bordered" id="dataTable" width="100%" cellspacing="0">
            ...
            <tbody>
                ...
                $total=0;
                    ...
                    $result = mysqli_query($conexion, "SELECT cast(SUM(r.cantidad * r.metros * e.peso)as decimal(10,2)) as totalpeso, r.id,cast((e.peso*r.metros)as decimal(10,2)) as pesoxunidad,c.nombre,i.producto,e.seccion as idseccion,r.cantidad,r.metros,r.unidad,c.id as id_cliente
                    FROM remitosclientestmp as r INNER JOIN inventario as i INNER JOIN clientes as c INNER JOIN equivalencias as e 
                    ON r.producto = i.id AND r.id_cliente = c.id AND e.id = r.id_seccion AND r.id_cliente = $id GROUP BY r.id ORDER BY r.id ASC");
                while ($fila = mysqli_fetch_assoc($result)) :                        
                ?>
                <tr>
                    <td width="20%"><?php echo $fila['nombre']; ?></td>
                    <td width="30%"><?php echo $fila['producto']; ?></td>
                    <td width="30%"><?php echo $fila['idseccion']; ?></td>
